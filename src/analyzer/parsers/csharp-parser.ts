...
class CSharpAstVisitor {
  ...
  private visitNode(node: Parser.SyntaxNode): boolean {
    // Return boolean to indicate if recursion should stop
    try {
      switch (node.type) {
      ...
      logger.warn(
        `[CSharpAstVisitor] Error visiting node type ${node.type} in ${this.filepath}: ${error.message}`,
      );
  ...
  private visitNamespaceDeclaration(node: Parser.SyntaxNode) {
    ...
    const nsNode: NamespaceDeclarationNode = {
      id: generateInstanceId(this.instanceCounter, "namespace", name, {
  ...
  private visitUsingDirective(node: Parser.SyntaxNode) {
    ...
    const alias = aliasNode
      ? getNodeText(aliasNode.childForFieldName("name"))
      : undefined;
    const isStatic = node.children.some(
      (c: Parser.SyntaxNode) => c.type === "static",
    );
    ...
    if (!namespaceOrType) {
      logger.warn(
        `[CSharpAstVisitor] Could not extract name for using_directive at ${this.filepath}:${location.startLine}`,
      );
      return;
    }
    ...
    const entityId = generateEntityId(
      "usingdirective",
      `${this.filepath}:${namespaceOrType}:${location.startLine}`,
    );
    const usingNode: UsingDirectiveNode = {
      id: generateInstanceId(this.instanceCounter, "using", namespaceOrType, {
        line: location.startLine,
        column: location.startColumn,
      }),
      entityId: entityId,
  ...
  private visitContainerDeclaration(
    node: Parser.SyntaxNode,
    kind: "CSharpClass" | "CSharpInterface" | "CSharpStruct",
  ) {
    const location = getNodeLocation(node);
    const nameNode = node.childForFieldName("name");
    const name = getNodeText(nameNode);
  ...
  private visitMethodDeclaration(node: Parser.SyntaxNode) {
    ...
    // Get the qualified name from the parent container node
    const parentNode = this.nodes.find(
      (n) => n.entityId === this.currentContainerId,
    );
    ...
    const methodNode: CSharpMethodNode = {
      id: generateInstanceId(this.instanceCounter, "csharpmethod", name, {
        line: location.startLine,
        column: location.startColumn,
      }),
    ...
    const relEntityId = generateEntityId(
      "has_method",
      `${this.currentContainerId}:${methodEntityId}`,
    );
    this.relationships.push({
      id: generateInstanceId(
        this.instanceCounter,
        "has_method",
        `${this.currentContainerId}:${methodNode.id}`,
      ),
      entityId: relEntityId,
      type: "HAS_METHOD",
      sourceId: this.currentContainerId,
  ...
  private visitPropertyDeclaration(node: Parser.SyntaxNode) {
    ...
    const propEntityId = generateEntityId(
      "property",
      `${parentQualifiedName}.${name}`,
    );
    const propNode: PropertyNode = {
      id: generateInstanceId(this.instanceCounter, "property", name, {
        line: location.startLine,
        column: location.startColumn,
      }),
    ...
    const relEntityId = generateEntityId(
      "has_property",
      `${this.currentContainerId}:${propEntityId}`,
    );
    this.relationships.push({
      id: generateInstanceId(
        this.instanceCounter,
        "has_property",
        `${this.currentContainerId}:${propNode.id}`,
      ),
      entityId: relEntityId,
      type: "HAS_PROPERTY",
      sourceId: this.currentContainerId,
      targetId: propEntityId,
      createdAt: this.now,
      weight: 7,
    });
  }
    ...
    for (const declarator of declarationNode.namedChildren) {
      if (declarator.type === "variable_declarator") {
        ...
        const fieldNode: FieldNode = {
          id: generateInstanceId(this.instanceCounter, "field", name, {
            line: location.startLine,
            column: location.startColumn,
          }),
        ...
        const relEntityId = generateEntityId(
          "has_field",
          `${this.currentContainerId}:${fieldEntityId}`,
        );
        this.relationships.push({
          id: generateInstanceId(
            this.instanceCounter,
            "has_field",
            `${this.currentContainerId}:${fieldNode.id}`,
          ),
          entityId: relEntityId,
          type: "HAS_FIELD",
          sourceId: this.currentContainerId,
...
export class CSharpParser {
  ...
  async parseFile(file: FileInfo): Promise<string> {
    ...
    try {
      ...
      logger.info(
        `[CSharpParser] Pass 1 completed for: ${file.name}. Nodes: ${result.nodes.length}, Rels: ${result.relationships.length}. Saved to ${path.basename(tempFilePath)}`,
      );
      return tempFilePath;
    } catch (error: any) {
      ...
      throw new ParserError(`Failed C# Pass 1 parsing for ${file.path}`, {
        originalError: error,
      });
    }
  }
