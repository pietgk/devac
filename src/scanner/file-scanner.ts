...
export class FileScanner {
  ...
  constructor(
    ...
    if (!path.isAbsolute(targetDirectory)) {
      throw new FileSystemError(
        "FileScanner requires an absolute target directory path.",
      );
    }
    ...
    if (isScanningFixtures) {
      ...
      baseIgnorePatterns = baseIgnorePatterns.filter(
        (pattern) => pattern !== "**/__tests__/**",
      );
    }
    // --- End Fix ---
    ...
    logger.debug("FileScanner initialized", {
      ...
      combinedIgnorePatterns: this.combinedIgnorePatterns,
    });
  ...
  async scan(): Promise<FileInfo[]> {
    ...
    try {
      ...
      logger.info(
        `Scan completed: ${foundFiles.length} files matching criteria found. Scanned ${scannedCount} total items. Encountered ${errorCount} errors.`,
      );
      return foundFiles;
    } catch (error: any) {
      logger.error(`Failed to scan directory: ${this.targetDirectory}`, {
        message: error.message,
      });
      throw new FileSystemError(
        `Failed to scan directory: ${this.targetDirectory}`,
        { originalError: error },
      );
    }
  }
  ...
  private async scanDirectoryRecursive(
    ...
    try {
      ...
      logger.warn(`Cannot read directory, skipping: ${currentPath}`, {
        code: error.code,
      });
      localErrorCount++;
