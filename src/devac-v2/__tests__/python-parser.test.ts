...
import { PythonParser, createPythonParser } from "../parsers/python-parser.js";
...
describe("PythonParser", () => {
  ...
  describe("class parsing", () => {
    ...
    it("extracts __init__ as method", async () => {
      ...
      const initMethods = result.nodes.filter(
        (n) => n.name === "__init__" && n.kind === "method"
      );
    ...
    it("handles multiple inheritance", async () => {
      ...
      const extendsEdges = result.edges.filter(
        (e) =>
          e.edge_type === "EXTENDS" &&
          e.source_entity_id === auditableClass?.entity_id
      );
  ...
  describe("function parsing", () => {
    ...
    it("extracts type hints on parameters", async () => {
      ...
      const addFunc = result.nodes.find(
      ...
      expect(
        addFunc?.return_type !== undefined ||
          result.edges.some(
            (e) =>
              e.edge_type === "RETURNS" &&
              e.source_entity_id === addFunc?.entity_id
          ) ||
  ...
  describe("import parsing", () => {
    it("extracts import statement as external ref", async () => {
      ...
      const osImport = result.externalRefs.find(
        (r) => r.module_specifier === "os"
      );
    ...
    it("extracts from...import statement", async () => {
      ...
      const typingImport = result.externalRefs.find(
        (r) =>
          r.module_specifier === "typing" && r.imported_symbol === "Optional"
      );
    ...
    it("handles import aliases", async () => {
      ...
      const jsonImport = result.externalRefs.find(
        (r) => r.module_specifier === "json"
      );
...
`;
      ...
      const relativeImports = result.externalRefs.filter(
        (r) => r.module_specifier?.startsWith(".")
      );
    ...
    it("handles star imports", async () => {
      ...
      const starImport = result.externalRefs.find(
        (r) =>
          r.module_specifier === "os.path" && r.imported_symbol === "*"
      );
  ...
  describe("variable parsing", () => {
    it("extracts module-level variables", async () => {
      ...
      const defaultService = variableNodes.find(
        (n) => n.name === "default_service"
      );
  ...
  describe("decorator parsing", () => {
    it("creates decorator nodes", async () => {
      ...
      const decoratedFunc = result.nodes.find(
        (n) => n.name === "decorated_function"
      );
      expect(decoratedFunc).toBeDefined();

      // Either we have DECORATES edges or decorator info in properties
      const decoratesEdges = result.edges.filter(
        (e) => e.edge_type === "DECORATES"
      );
      const hasDecoratorInfo =
        decoratesEdges.length > 0 ||
        decoratedFunc?.properties?.decorators?.length > 0;
  ...
  describe("scope handling", () => {
    it("generates correct scoped names for methods", async () => {
      ...
      const methodNode = result.nodes.find(
        (n) => n.kind === "method" && n.name === "get_user"
      );
...
`;
      ...
      const innerClass = result.nodes.find(
        (n) => n.kind === "class" && n.name === "Inner"
      );
...
`;
    ...
    it("handles non-existent file gracefully", async () => {
      await expect(
        parser.parse("/non/existent/file.py", testConfig)
      ).rejects.toThrow();
    });
  });
