...
import { createNode, createEdge, createExternalRef } from "../types/index.js";
import {
  generateEntityId,
  type EntityIdComponents,
} from "../analyzer/entity-id-generator.js";
...
export class TypeScriptParser implements LanguageParser {
  ...
  private handleClass(
    ...
    ctx.result.edges.push(
      ctx.createContainsEdge(fileEntityId, classNode.entity_id, node),
    );
  ...
  private handleClassMethod(
    ...
    ctx.result.edges.push(
      ctx.createContainsEdge(classEntityId, methodNode.entity_id, node),
    );
  }
  ...
  private handleClassProperty(
    ...
    ctx.result.edges.push(
      ctx.createContainsEdge(classEntityId, propNode.entity_id, node),
    );
  }
  ...
  private handleFunction(
    ...
    ctx.result.edges.push(
      ctx.createContainsEdge(fileEntityId, funcNode.entity_id, node),
    );
  }
  ...
  private handleImport(
    ...
    for (const specifier of node.specifiers) {
      ...
      const specifierIsTypeOnly =
        ...
        (specifier as t.ImportSpecifier & { importKind?: string })
          .importKind === "type";
  ...
  private handleInterface(
    ...
    ctx.result.edges.push(
      ctx.createContainsEdge(fileEntityId, interfaceNode.entity_id, node),
    );
  ...
  private handleTypeAlias(
    ...
    ctx.result.edges.push(
      ctx.createContainsEdge(fileEntityId, typeNode.entity_id, node),
    );
  }
  ...
  private handleEnum(
    ...
    ctx.result.edges.push(
      ctx.createContainsEdge(fileEntityId, enumNode.entity_id, node),
    );
    ...
    for (const member of node.members) {
      if (t.isIdentifier(member.id)) {
        ...
        ctx.result.edges.push(
          ctx.createContainsEdge(
            enumNode.entity_id,
            memberNode.entity_id,
            member,
          ),
        );
      }
  ...
  private isExported(nodePath: NodePath): boolean {
    const parent = nodePath.parent;
    return (
      t.isExportNamedDeclaration(parent) || t.isExportDefaultDeclaration(parent)
    );
  }
