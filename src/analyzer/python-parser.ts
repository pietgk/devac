...
import {
  ...
  generateInstanceId,
  generateEntityId,
} from "./parser-utils.js"; // Reusing utils
...
export class PythonAstParser {
  ...
  constructor(pythonExecutable: string = "python3") {
    ...
    logger.debug(
      `Python AST Parser initialized with executable: ${this.pythonExecutable}`,
    );
  }
    ...
    try {
      ...
      if (result.error) {
        throw new ParserError(
          `Python script reported error for ${file.path}: ${result.error}`,
        );
      }
      ...
      if (
        ...
        throw new ParserError(
          `Invalid JSON structure received from python_parser.py for ${file.path}`,
        );
      }

      // --- DEBUG LOG: Inspect raw result ---
      logger.debug(
        `[PythonAstParser] Raw result from python_parser.py for ${file.name}: ${JSON.stringify(result, null, 2)}`,
      );
      // --- END DEBUG LOG ---
      ...
      const finalResult: SingleFileParseResult = {
        ...
        nodes: result.nodes.map((node) => ({
          ...
          id: generateInstanceId(
            ...
            node.name,
            { line: node.startLine, column: node.startColumn },
          ),
      ...
      logger.info(
        `[PythonAstParser] Pass 1 completed for: ${file.name}. Nodes: ${finalResult.nodes.length}, Rels: ${finalResult.relationships.length}. Saved to ${path.basename(tempFilePath)}`,
      );
      return tempFilePath;
    } catch (error: any) {
      logger.error(
        `[PythonAstParser] Error during Python Pass 1 for ${file.path}`,
        {
      ...
      throw new ParserError(`Failed Python Pass 1 parsing for ${file.path}`, {
        originalError: error,
      });
    }
  }
  ...
  private runPythonScript(filePath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      logger.debug(
        `[PythonAstParser] Node.js check: File '${filePath}' exists? ${fileExists}`,
      );
      if (!fileExists) {
        return reject(
          new ParserError(
            `Node.js cannot find the file before spawning Python: ${filePath}`,
          ),
        );
      }
      // --- End Debug ---
      ...
      logger.debug(
        `[PythonAstParser] Executing: ${this.pythonExecutable} "${scriptPath}" "${filePath}"`,
      );
      ...
      childProcess.on("error", (err) => {
        ...
        reject(
          new ParserError(
            `Failed to start python script '${this.pythonExecutable}'. Is Python installed and in PATH?`,
            { originalError: err },
          ),
        );
      });
      ...
      childProcess.on("close", (code) => {
        ...
        if (code === 0) {
          if (stderrData) {
            logger.warn(
              `[PythonAstParser] Python script produced stderr output (but exited OK): ${stderrData.trim()}`,
            );
          }
          resolve(stdoutData);
        } else {
          ...
          try {
            ...
            if (errorJson.error) {
              reject(
                new ParserError(`Python script error: ${errorJson.error}`),
              );
              return;
            }
          } catch {
            /* Ignore JSON parse error on stderr */
          }
          // Fallback error
          reject(
            new ParserError(
              `Python script exited with code ${code}. Stderr: ${stderrData.trim()}`,
            ),
          );
        }
      });
