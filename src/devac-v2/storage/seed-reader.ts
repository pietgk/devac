...
import { DuckDBPool, executeWithRecovery } from "./duckdb-pool.js";
...
import { getSeedPaths, type SeedPaths } from "../types/config.js";
...
export class SeedReader {
   ...
   */
  async readNodes(branch: string = "base"): Promise<QueryResult<ParsedNode>> {
   ...
   */
  async readEdges(branch: string = "base"): Promise<QueryResult<ParsedEdge>> {
    const startTime = Date.now();
    const paths = getSeedPaths(this.packagePath, branch);
   ...
   */
  async readExternalRefs(
    branch: string = "base",
  ): Promise<QueryResult<ParsedExternalRef>> {
    const startTime = Date.now();
    const paths = getSeedPaths(this.packagePath, branch);
   ...
   */
  async querySeeds<T = Record<string, unknown>>(
    sql: string,
   ...
   */
  async getUnresolvedRefs(
    branch: string = "base",
  ): Promise<ParsedExternalRef[]> {
    const paths = getSeedPaths(this.packagePath, branch);
   ...
   */
  async getFileHashes(branch: string = "base"): Promise<Map<string, string>> {
    const paths = getSeedPaths(this.packagePath, branch);
    const basePath = path.join(paths.basePath, "nodes.parquet");
   ...
   */
  async validateIntegrity(branch: string = "base"): Promise<IntegrityResult> {
    const paths = getSeedPaths(this.packagePath, branch);
    const errors: string[] = [];
      ...
      if (await this.fileExists(edgesPath)) {
        try {
          ...
          if (await this.fileExists(nodesPath)) {
            ...
            `);
            const orphanRow = orphanCheck[0] as
              | Record<string, unknown>
              | undefined;
            orphanedEdges = Number(orphanRow?.count ?? 0);
            if (orphanedEdges > 0) {
              warnings.push(
                `Found ${orphanedEdges} edges with missing source nodes`,
              );
            }
          }
        } catch (error) {
   ...
   */
  async getStats(branch: string = "base"): Promise<IntegrityResult["stats"]> {
 ...
 */
export function createSeedReader(
  pool: DuckDBPool,
  packagePath: string,
): SeedReader {
...
export async function queryMultiplePackages<T = Record<string, unknown>>(
  ...
  let processedSql = sql
    .replace(
      /{nodes}/g,
      `read_parquet([${nodePaths.map((p) => `'${p}'`).join(", ")}])`,
    )
    .replace(
      /{edges}/g,
      `read_parquet([${edgePaths.map((p) => `'${p}'`).join(", ")}])`,
    )
    .replace(
      /{external_refs}/g,
      `read_parquet([${refPaths.map((p) => `'${p}'`).join(", ")}])`,
    );
