...
import { computeStringHash, computeFileHash } from "../utils/hash.js";
import type {
  LanguageParser,
  ParserConfig,
...
export class PythonParser implements LanguageParser {
  ...
  async parse(
    ...
    const result = this.convertOutput(
      pythonOutput,
      filePath,
      sourceFileHash,
      config,
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on("close", (code) => {
        if (code !== 0) {
          ...
          reject(
            new Error(
              `Python parser exited with code ${code}: ${stderr || stdout}`,
            ),
          );
          return;
        }
  ...
  private convertOutput(
    ...
    const edges: ParsedEdge[] = output.edges.map((raw) =>
      this.convertEdge(raw, sourceFileHash),
    );
