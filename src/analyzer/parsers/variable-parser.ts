...
} from "ts-morph"; // Import VariableDeclarationKind
...
export function parseVariables(context: ParserContext): void {
  ...
  logger.debug(
    `Found ${variableStatements.length} variable statements in ${fileNode.name}`,
  );
  ...
  for (const statement of variableStatements) {
    ...
    for (const declaration of declarations) {
      try {
        ...
        // Generate simple entity ID for variable
        ...
        logger.warn(
          `Error parsing variable declaration ${declaration.getName()} in ${fileNode.filePath}`,
          { message: e.message },
        );
      }
    }
