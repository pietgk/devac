...
const CROSS_FILE_RELATIONSHIP_TYPES = BASE_RELATIONSHIP_TYPES.filter(
...
const nodeUniquenessConstraints = NODE_LABELS.map(
  (label) =>
...
const indexes = [
  // CRITICAL: Node base label index for unlabeled MERGE operations
  ...
  ...NODE_LABELS.map(
    (label) =>
      `CREATE INDEX ${label.toLowerCase()}_name_index IF NOT EXISTS FOR (n:${label}) ON (n.name)`,
  ),
...
export class SchemaManager {
  ...
  async applySchema(forceUpdate: boolean = false): Promise<void> {
    ...
    for (const command of allSchemaCommands) {
      try {
        ...
        logger.debug(
          `Successfully applied schema command: ${command.split(" ")[2]}...`,
        );
        appliedCount++;
      } catch (error: any) {
        const alreadyExists =
        ...
        if (!alreadyExists || forceUpdate) {
          logger.error(`Failed to apply schema command: ${command}`, {
            code: error.code,
          ...
          logger.debug(
            `Schema element already exists, skipping: ${command.split(" ")[2]}...`,
          );
        }
      }
    }
    logger.info(
      `Schema application finished. Applied/Verified: ${appliedCount}, Failed: ${failedCount}.`,
    );
    if (failedCount > 0 && forceUpdate) {
      throw new Neo4jError(
        `Failed to apply ${failedCount} schema elements during forced update.`,
      );
    }
  }
  ...
  async dropAllSchemaElements(): Promise<void> {
    logger.warn(
      "Dropping ALL user-defined constraints and indexes from the database...",
    );
    ...
    try {
      const constraintsResult =
        await this.neo4jClient.runTransaction<QueryResult>(
      ...
      const constraintNames =
        constraintsResult.records?.map((r: any) => r.get("name")) || [];
      ...
      for (const name of constraintNames) {
        try {
          ...
          logger.error(`Failed to drop constraint: ${name}`, {
            message: error.message,
          });
          failedDrops++;
        }
      ...
      const indexesResult = await this.neo4jClient.runTransaction<QueryResult>(
      ...
      const indexNames =
        indexesResult.records
          ?.map((r: any) => r.get("name"))
          .filter((name: string) => !name.includes("constraint")) || [];
      ...
      logger.info(
        `Finished attempting to drop schema elements: ${droppedConstraints} constraints, ${droppedIndexes} indexes dropped. ${failedDrops} failures.`,
      );
    } catch (error: any) {
      logger.error(
        "Failed to retrieve existing schema elements for dropping.",
        { message: error.message },
      );
      throw new Neo4jError("Failed to retrieve schema for dropping.", {
        originalError: error,
      });
    }
    if (failedDrops > 0) {
      logger.warn(
        `Encountered ${failedDrops} errors while dropping schema elements.`,
      );
    }
  }
  ...
  async resetDatabase(): Promise<void> {
    ...
    try {
      ...
      throw new Neo4jError("Failed to reset database.", {
        originalError: error,
      });
    }
  }
