...
describe("FileWatcher - Unit Tests", () => {
  ...
  describe("Debouncing", () => {
    it("should debounce multiple rapid changes", async () => {
      ...
      for (let i = 1; i <= writeCount; i++) {
        await fs.writeFile(testFile, `Change ${i}`, "utf-8");
        // Very short delay to keep all writes within the same debounce window
    ...
    it("should emit batched events after debounce window", async () => {
      ...
      await fs.writeFile(
        path.join(testWatchDir, "file1.txt"),
        "File 1",
        "utf-8",
      );
      await fs.writeFile(
        path.join(testWatchDir, "file2.txt"),
        "File 2",
        "utf-8",
      );
      await fs.writeFile(
        path.join(testWatchDir, "file3.txt"),
        "File 3",
        "utf-8",
      );
  ...
  describe("Ignore Patterns", () => {
    it("should ignore files matching ignore patterns", async () => {
      ...
      await fs.writeFile(
        path.join(testWatchDir, "app.log"),
        "Log file",
        "utf-8",
      );
      await fs.writeFile(
        path.join(testWatchDir, "data.json"),
        "Data file",
        "utf-8",
      );
      ...
      expect(receivedEvents.some((e) => e.path.includes("app.log"))).toBe(
        false,
      );
      expect(receivedEvents.some((e) => e.path.includes("data.json"))).toBe(
        true,
      );
    });
  });
  ...
  describe("Disposable Pattern", () => {
    ...
    it("should not emit events after disposal", async () => {
      ...
      await fs.writeFile(
        path.join(testWatchDir, "after-dispose.txt"),
        "Test",
        "utf-8",
      );
      await new Promise((resolve) => setTimeout(resolve, 400));
