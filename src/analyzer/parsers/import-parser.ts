...
export function parseImports(context: ParserContext): void {
  ...
  logger.debug(
    `Found ${importDeclarations.length} import declarations in ${fileNode.name}`,
  );

  for (const declaration of importDeclarations) {
    try {
      const moduleSpecifier =
        declaration.getModuleSpecifierValue() ?? "unknown_module";
      ...
      const importNode: AstNode = {
        ...
        endColumn: declaration.getEnd() - declaration.getStartLinePos(),
        properties: {
          ...
          // Store individual imported names for resolution
          importedNames: [
            ...namedImports,
            ...(defaultImport ? [defaultImport] : []),
            ...(namespaceImport ? [namespaceImport] : []),
        ...
        documentation: getJsDocDescription(declaration),
        createdAt: now,
      };
      addNode(importNode);
      ...
      logger.warn(
        `Failed to process import declaration at line ${declaration.getStartLineNumber()} in ${fileNode.filePath}: ${error.message}`,
      );
    }
  }
}
