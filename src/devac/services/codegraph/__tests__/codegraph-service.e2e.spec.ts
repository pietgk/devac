...
describe("CodeGraphService - E2E Tests (Trophy Top)", () => {
  ...
  describe("E1: Complete Lifecycle", () => {
    it("should handle complete lifecycle: init → scan → watch → process → cleanup", async () => {
      ...
      // Mock only analyzer (expensive operation) - must be after initialize()
      vi.spyOn(service["analyzerService"], "analyze").mockResolvedValue(
        undefined,
      );
      ...
      await expect(
        fs.access(path.join(tempDir, "logs")),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(tempDir, "resources")),
      ).resolves.toBeUndefined();
      ...
      const stopWatcher = service["startWatcher"]((e) =>
      ...
      expect(output.stats.duration).toBeGreaterThanOrEqual(0); // Duration can be 0 for very fast operations
      ...
      const finalCollections = await queryCollections(
        neo4jClient,
        "test-codegraph",
      );
    ...
    it("should handle errors gracefully throughout lifecycle", async () => {
      ...
      // Mock analyzer to fail - must be after initialize()
      ...
      const finalServiceNode = await queryServiceNode(
        neo4jClient,
        "test-codegraph",
      );
