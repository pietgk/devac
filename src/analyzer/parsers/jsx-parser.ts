  ...
  for (const jsxNode of allJsxNodes) {
    try {
      ...
      if (isSelfClosing) {
        ...
        logger.warn(
          `[jsx-parser] Encountered unexpected JSX node type in ${fileNode.filePath}`,
        );
        continue;
      }
      ...
      const jsxElementNode: JSXElementNode = {
        id: generateId("jsxelement", jsxInstanceId, {
      ...
      logger.debug(
        `[jsx-parser] Added JSXElement node: ${tagName} (EntityId: ${jsxElementEntityId})`,
      );
      ...
      while (currentParent) {
        const potentialComp = result.nodes.find(
          (n) =>
            n.kind === "Component" &&
        ...
        if (parentJsxNode) {
          ...
          if (parentTagNameNode) {
            ...
            const parentLocation = {
              ...
              startColumn:
                parentJsxNode.getStart() - parentJsxNode.getStartLinePos(),
            };
            const parentInstanceId = `${fileNode.filePath}:${parentTagName}:${parentLocation.startLine}:${parentLocation.startColumn}`;
            const parentEntityId = generateEntityId(
              "jsxelement",
              parentInstanceId,
            );
            const foundParentJsxAstNode = result.nodes.find(
              (n) => n.entityId === parentEntityId,
      ...
      if (parentAstNode) {
        const relEntityId = generateEntityId(
          "renders_element",
          `${parentAstNode.entityId}:${jsxElementEntityId}`,
        );
        addRelationship({
          id: generateId(
            "renders_element",
            `${parentAstNode.id}:${jsxElementNode.id}`,
          ),
          entityId: relEntityId,
          type: "RENDERS_ELEMENT",
          sourceId: parentAstNode.entityId,
          targetId: jsxElementEntityId,
          weight: 6,
          createdAt: now,
        });
        logger.debug(
          `[jsx-parser] Added RENDERS_ELEMENT relationship: ${parentAstNode.name} -> ${jsxElementNode.name}`,
        );
      } else {
        logger.debug(
          `[jsx-parser] Could not find parent Component or JSXElement for ${jsxElementNode.name} at line ${jsxElementNode.startLine}`,
        );
      }
      ...
      for (const attribute of attributes) {
        if (Node.isJsxAttribute(attribute)) {
          ...
          const attrLocation = {
            ...
            endLine: attribute.getEndLineNumber(),
            startColumn: attribute.getStart() - attribute.getStartLinePos(),
            endColumn: 0,
          };
          ...
          const jsxAttributeNode: JSXAttributeNode = {
            id: generateId("jsxattribute", attrInstanceId, {
              line: attrLocation.startLine,
              column: attrLocation.startColumn,
            }),
            ...
            filePath: fileNode.filePath,
            startLine: attrLocation.startLine,
            endLine: attrLocation.endLine,
            language: "TypeScript", // Assuming TS/JS context
            startColumn: attrLocation.startColumn,
            endColumn: attrLocation.endColumn,
            createdAt: now,
            properties: { value: attrValue },
          };
          ...
          const relEntityId = generateEntityId(
            "has_prop",
            `${jsxElementEntityId}:${attrEntityId}`,
          );
          addRelationship({
            id: generateId(
              "has_prop",
              `${jsxElementNode.id}:${jsxAttributeNode.id}`,
            ),
            entityId: relEntityId,
            type: "HAS_PROP",
            sourceId: jsxElementEntityId,
            targetId: attrEntityId,
            weight: 7,
            createdAt: now,
          });
          logger.debug(
            `[jsx-parser] Added HAS_PROP relationship: ${jsxElementNode.name} -> ${jsxAttributeNode.name}`,
          );

          // --- Tailwind CSS Class Parsing ---
          if (attrName === "className" && typeof attrValue === "string") {
            // Skip dynamic className values (template literals, JSX expressions)
            if (
              attrValue.includes("{") ||
              attrValue.includes("$") ||
              attrValue.includes("`")
              ...
              logger.debug(
                `[jsx-parser] Skipping dynamic className value: "${attrValue}"`,
              );
              continue;
            }

            const classNames = attrValue.split(/\s+/).filter((cn) => {
              const trimmed = cn.trim();
              // Filter out empty strings, expressions, and suspiciously long values
              return (
                trimmed !== "" &&
                !trimmed.includes("{") &&
                !trimmed.includes("$") &&
            ...
            logger.debug(
              `[jsx-parser] Found className attribute with ${classNames.length} static classes: "${attrValue}"`,
            );

            for (const className of classNames) {
              try {
                const tailwindClassName = className;
                const tailwindEntityId = generateEntityId(
                  "tailwindclass",
                  tailwindClassName,
                );
                ...
                if (!tailwindNode) {
                  tailwindNode = {
                    ...
                    id: generateId("tailwindclass", tailwindClassName, {
                      line: attrLocation.startLine,
                      column: attrLocation.startColumn,
                    }),
                  ...
                  logger.debug(
                    `[jsx-parser] Added TailwindClass node: ${tailwindClassName}`,
                  );
                }

                // Ensure tailwindNode exists before creating relationship
                if (tailwindNode) {
                  const twRelEntityId = generateEntityId(
                    "uses_tailwind_class",
                    `${jsxElementEntityId}:${tailwindEntityId}`,
                  );
                  addRelationship({
                    // Now tailwindNode is guaranteed to be defined here
                    id: generateId(
                      "uses_tailwind_class",
                      `${jsxElementNode.id}:${tailwindNode.id}`,
                    ),
                    entityId: twRelEntityId,
                    type: "USES_TAILWIND_CLASS",
                    sourceId: jsxElementEntityId,
                    targetId: tailwindEntityId,
                  ...
                  logger.debug(
                    `[jsx-parser] Added USES_TAILWIND_CLASS relationship: ${jsxElementNode.name} -> ${tailwindNode.name}`,
                  );
                } else {
                  // This case should not happen if cache logic is correct, but log just in case
                  logger.warn(
                    `[jsx-parser] Could not find or create Tailwind node for class "${tailwindClassName}" before relationship creation.`,
                  );
                }
              } catch (tailwindError: any) {
                logger.warn(
                  `[jsx-parser] Error processing Tailwind class "${className}" in ${fileNode.filePath}: ${tailwindError.message}`,
                );
              }
            }
          }
      ...
      const tagName = Node.isJsxSelfClosingElement(jsxNode)
        ...
        : Node.isJsxElement(jsxNode)
          ? jsxNode.getOpeningElement().getTagNameNode().getText()
          : "unknown";
      logger.error(
        `[jsx-parser] Error processing JSX element "${tagName}" in ${fileNode.filePath}: ${error.message}`,
