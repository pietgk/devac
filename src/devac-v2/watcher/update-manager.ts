...
import type {
  LanguageParser,
  ParserConfig,
} from "../parsers/parser-interface.js";
import { createSeedWriter, SeedWriter } from "../storage/seed-writer.js";
...
class UpdateManagerImpl implements UpdateManager {
  ...
  constructor(
    ...
    this.writer = writer;
    this.parser = parser;
    this.logger = createLogger("UpdateManager");
  }
  ...
  private async handleAddOrChange(
    ...
    try {
      ...
      // Write seeds
      const writeResult = await this.writer.updateFile(
        [relativePath],
        parseResult,
        { branch: this.config.branch },
      );
 ...
 */
export async function createUpdateManager(
  config: UpdateManagerConfig,
