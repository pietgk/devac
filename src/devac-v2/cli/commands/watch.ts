...
import { createFileWatcher, FileWatcher } from "../../watcher/file-watcher.js";
import {
  createUpdateManager,
  UpdateManager,
} from "../../watcher/update-manager.js";
import {
  createRenameDetector,
  RenameDetector,
} from "../../watcher/rename-detector.js";
import { createLogger, Logger, setLogLevel } from "../../utils/logger.js";

/**
 * Default watch options
 */
const DEFAULT_OPTIONS: Required<
  Omit<WatchOptions, "packagePath" | "repoName">
> = {
  branch: "base",
  debounceMs: 100,
  force: false,
...
class WatchControllerImpl implements WatchController {
  ...
  constructor(options: WatchOptions) {
    ...
    this.logger = createLogger("WatchCommand");
    ...
    try {
      ...
      if (needsAnalysis) {
        ...
        if (analyzeResult.success) {
          ...
          this.logger.info(
            `Initial analysis complete: ${analyzeResult.filesAnalyzed} files`,
          );
        } else {
          this.logger.warn(
            `Initial analysis had issues: ${analyzeResult.error}`,
          );
        }
      } else {
    ...
    this.fileWatcher.on("batch", async (events) => {
      ...
      try {
        ...
        for (const rename of processed.renames) {
          this.logger.debug(
            `Detected rename: ${rename.oldPath} -> ${rename.newPath}`,
          );
          try {
        ...
        for (const event of remainingEvents) {
          ...
          try {
            ...
            if (!result.success && !result.skipped) {
              this.status.errors++;
              this.logger.warn(
                `Failed to process ${event.filePath}: ${result.error}`,
              );
            }
            ...
            this.status.changesProcessed++;
            this.logger.warn(
              `Error processing ${event.filePath}: ${eventError}`,
            );
          }
