...
export class Neo4jClient {
  ...
  public async initializeDriver(context: string = "Default"): Promise<void> {
    ...
    try {
        ...
        neo4j.auth.basic(this.neo4jConfig.username, this.neo4jConfig.password),
        {
          ...
          logging: {
            ...
            logger: (level, message) =>
              logger.log(level, `(neo4j-driver) ${message}`),
          },
        },
      );
      ...
      await this.verifyConnectivity(context);
      logger.info(
        `(${context}) Successfully connected to Neo4j database: ${this.neo4jConfig.database}`,
      );
    } catch (error: any) {
      logger.error(
        `(${context}) Failed to initialize Neo4j driver or connect to database.`,
        {
      ...
      throw new Neo4jError(`Failed to connect to Neo4j: ${error.message}`, {
        originalError: error,
      });
    }
  }
  ...
  private async verifyConnectivity(context: string): Promise<void> {
    if (!this.driver) {
      throw new Neo4jError(
        "Driver not initialized. Cannot verify connectivity.",
      );
    }
    logger.info(
  ...
  public async getDriver(context: string = "Default"): Promise<Driver> {
    if (!this.driver) {
      logger.warn(
        `(${context}) Neo4j driver not connected. Attempting connection via getDriver...`,
      );
      await this.initializeDriver(context);
      logger.info(
        `(${context}) Neo4j driver connection successful via getDriver.`,
      );
    }
    // We check driver again because initializeDriver could throw
    if (!this.driver) {
      throw new Neo4jError(
        "Failed to get Neo4j driver after initialization attempt.",
      );
    }
    return this.driver;
  }
  ...
  public async getSession(
    ...
    try {
      ...
      logger.error(`(${context}) Failed to obtain Neo4j session.`, {
        error: error.message,
      });
      throw new Neo4jError(`Failed to obtain Neo4j session: ${error.message}`, {
        originalError: error,
      });
    }
  }
  ...
  public async runTransaction<T>(
    ...
    try {
      ...
      logger.error(
        `(${context}) Error executing Neo4j transaction. Cypher: ${cypher.substring(0, 100)}...`,
        {
          error: error.message,
      ...
      if (session) {
        try {
          ...
          logger.error(`(${context}) Failed to close Neo4j session.`, {
   ...
   */
  public async isConnectionHealthy(
    context: string = "Default",
  ): Promise<boolean> {
    if (!this.driver) {
      logger.debug(
        `(${context}) Connection health check: driver not initialized`,
      );
      return false;
    }
    ...
    try {
      ...
      logger.debug(`(${context}) Connection health check: healthy`);
      return true;
    } catch (error: any) {
      logger.warn(
        `(${context}) Connection health check failed: ${error.message}`,
      );
      return false;
    }
  }
  ...
  /**
   * Reconnects to Neo4j after connection loss (e.g., after system sleep).
   * Closes existing driver and creates a new one.
   * @param context - Optional context string for logging.
   * @throws {Neo4jError} If reconnection fails.
   */
  public async reconnect(context: string = "Default"): Promise<void> {
    logger.info(`(${context}) Attempting to reconnect to Neo4j...`);
    ...
    if (this.driver) {
      try {
        ...
        this.driver = null;
        logger.debug(`(${context}) Closed stale driver connection`);
      } catch (error: any) {
        logger.warn(
          `(${context}) Error closing stale driver: ${error.message}`,
        );
        this.driver = null; // Force null even if close fails
      }
    }

    // Initialize new driver
    await this.initializeDriver(context);
    logger.info(`(${context}) Successfully reconnected to Neo4j`);
  }
  ...
  public async closeDriver(context: string = "Default"): Promise<void> {
    if (this.driver) {
      ...
      try {
        ...
        logger.error(`(${context}) Error closing Neo4j driver.`, {
          error: error.message,
        });
      ...
      logger.debug(
        `(${context}) Neo4j driver already closed or not initialized.`,
      );
    }
  }
}
