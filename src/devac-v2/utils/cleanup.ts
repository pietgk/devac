...
export async function cleanupPackageSeeds(
  ...
  if (opts.removeStaleLocks) {
    ...
    try {
      ...
      result.errors.push(`Failed to check/remove lock: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  if (opts.removeTempFiles) {
    ...
    try {
      ...
      result.errors.push(`Failed to clean temp files: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
export async function removeAllSeeds(
  ...
  try {
    ...
    errors.push(`Failed to remove .devac: ${error instanceof Error ? error.message : String(error)}`);
  }
 ...
 * @param packagePath - Path to the package
 */
export async function getSeedStorageStats(
  packagePath: string
): Promise<{
  ...
  async function walkDir(dirPath: string, isBase: boolean = false, isBranch: boolean = false): Promise<void> {
    try {
